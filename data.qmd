...
xls <- readxl::excel_sheets(file_path)

# Loop through each sheet and create visualizations
for (sheet in xls) {
    data <- readxl::read_excel(file_path, sheet = sheet)
    
    # Heatmap of Missing Values for each sheet
    vis_miss(data) + ggtitle(paste("Missing Data Heatmap -", sheet))
    
    # Bar Plot of Missing Values by Variable for each sheet
    data %>%
        ...
        theme_minimal() +
        labs(title = paste("Proportion of Missing Data by Variable -", sheet), x = "Variable", y = "Percentage Missing")
}
